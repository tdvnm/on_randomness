...
all of them have to be numbers an attacker cannot guess. But people mean two
different things by ``random'', and it is easy to mix them up.
\begin{enumerate}
...
\begin{itemize}
  ...
  \item the Blum--Blum--Shub generator~\cite{blum1986}.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Runs test.} Count the runs of identical bits and compare with
        ...
        idea~\cite{wald1940}).
  \item \textbf{Chi-square test on pairs.} The four patterns $00, 01, 10, 11$
        should each show up a quarter of the time. Pearson's
        statistic~\cite{pearson1900} measures how far off they are.
  \item \textbf{Autocorrelation.} The correlation between $b_i$ and
        $b_{i+k}$ for small $k$. It should be close to zero.
...
\begin{description}
  ...
  \item[B. Biased key.] One key value much more likely than the others, say
        probability $0.7$. I expect $\MI{\msg}{\ctxt} > 0$.
  \item[C. Sweep.] Each key bit is $1$ with probability
...
\end{description}
Part C gives the main figure of the project: leakage plotted against key
...
random and is not secure. The Blum--Blum--Shub generator~\cite{blum1986} is
the contrast: it was built so that predicting the next bit is as hard as a
number-theory problem, not just so that the output looks nice. Knuth's
chapter on random numbers~\cite{knuth1997} and Katz and Lindell's
textbook~\cite{katz2020} both make this distinction, and this part follows
them.
...
I want to \emph{use} the course material, not repeat it.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.30\linewidth}>{\raggedright\arraybackslash}p{0.44\linewidth}>{\raggedright\arraybackslash}p{0.20\linewidth}@{}}
    ...
    Covariance and correlation
      & Lag-$k$ autocorrelation of a bit sequence
      & \S7.3 \\
    Multinomial
      & Counts of the pairs $00,01,10,11$ that feed the chi-square test
      & \S7.4 \\
    ...
    Chi-square distribution
      & Null distribution of Pearson's statistic
      & \S10.4 \\
    Markov chains
...
My frequency, runs and pair-frequency tests are simplified versions of tests
in this suite. I follow its conventions for $p$-values.

\paragraph{Blum, Blum and Shub (1986)~\cite{blum1986}.}
The generator I use as the example of real unpredictability, as opposed to
merely looking random.
...
\paragraph{Pearson (1900)~\cite{pearson1900} and Wald and Wolfowitz
  (1940)~\cite{wald1940}.}
The original papers for the chi-square goodness-of-fit test and the runs test.
...
To check that the notation above actually works, here is the one derivation
the whole project hangs on, done for the smallest case I will use.
...
\]
That is Shannon's condition exactly, and it holds for \emph{every} message
distribution, including the lopsided one in Part~III. The three tools used
are conditional probability, the law of total probability and Bayes' rule,
all from Chapter~2 of the course textbook.

The same computation shows what goes wrong when the key is not uniform. If
$\Prob{\key = k}$ varies with $k$, then
$\Prob{\ctxt = c \mid \msg = m} = \Prob{\key = m \xor c}$ varies with $m$, so
the ciphertext distribution depends on the message and
$\Prob{\msg = m \mid \ctxt = c} \neq \Prob{\msg = m}$ in general. Part~III
measures \emph{how much} it differs, using $\MI{\msg}{\ctxt}$.

\section{Plan of work}\label{sec:plan}

Table~\ref{tab:plan} is the order I intend to do things in. The simulations
come before the writing in each part, so that the report describes results I
actually have.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lp{0.62\linewidth}@{}}
    ...
    1 & Set up the code: bit-sequence sources (coins, LCG, OS generator,
        BBS) and the five tests. Check each test on a known-fair source
    ...
    4 & Main experiment (A, B and the sweep C); exact mutual information
        for the two-bit system to compare against; the main figure. \\
    5 & Part~IV: recover an LCG's state from its outputs; contrast with
        BBS. Interactive program. \\
